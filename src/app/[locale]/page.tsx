...
export default async function Home({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [massSchedules, specialMasses, gallery, socialLinks, eventsSection, homeContent] =
    (await Promise.all([
      getMassSchedules(),
      getSpecialMasses(),
      getGalleryPhotos(6),
      ...
      MassScheduleRow[],
      SpecialMassRow[],
      Awaited<ReturnType<typeof getGalleryPhotos>>,
      Awaited<ReturnType<typeof getActiveSocialLinks>>,
      Awaited<ReturnType<typeof getEventsSection>>,
      Awaited<ReturnType<typeof getHomeContentMap>>,
    ]
  ...
  massSchedules.forEach((schedule) => {
    const locationName = schedule.location?.name || t('unassignedLocation')
  ...
  specialMasses.forEach((special) => {
    const locationName = special.location || t('unassignedLocation')
    const current = scheduleByLocation.get(locationName)
    if (current) {
    ...
    <div className="min-h-screen">
      ...
      <section className="py-20 bg-white text-center">
        <div className="max-w-4xl mx-auto px-4 sm:px-5">
          <span className="text-sm font-semibold text-[var(--primary)] uppercase tracking-widest">{t('welcome')}</span>
      ...
      {eventsSection && (
        <section className="py-20 bg-gray-50">
          <div className="max-w-4xl mx-auto px-4 sm:px-5">
            <span className="text-sm font-semibold text-[var(--primary)] uppercase tracking-widest">{t('events')}</span>
            <h2 className="text-3xl md:text-4xl my-6 text-[var(--secondary)]" style={{ fontFamily: 'var(--font-heading)' }}>
              {eventsSection.title || t('events')}
            </h2>
            <div
      ...
      >
        <div className="max-w-4xl mx-auto px-4 sm:px-5">
          <span className="text-sm font-semibold text-[var(--primary)] uppercase tracking-widest">{t('massSchedule')}</span>
          <h2 className="text-3xl md:text-4xl my-6 text-[var(--secondary)]" style={{ fontFamily: 'var(--font-heading)' }}>
            {t('joinWorship')}
          </h2>
                ...
                return (
                  <div key={locationCard.name} className="bg-white p-8 rounded-lg shadow-lg text-left">
                    ...
                    {sundayMasses.length > 0 && (
                      <div className="mb-4">
                        <h4 className="font-semibold text-[var(--secondary)]">{t('sundays')}</h4>
                    ...
                    {dailyMasses.length > 0 && (
                      <div className="mb-4">
                        <h4 className="font-semibold text-[var(--secondary)]">{t('dailyMasses')}</h4>
                    ...
                    {adorationMasses.length > 0 && (
                      <div className="mb-4">
                        <h4 className="font-semibold text-[var(--secondary)]">{t('adoration')}</h4>
                    ...
                    {confessionMasses.length > 0 && (
                      <div className="mb-4">
                        <h4 className="font-semibold text-[var(--secondary)]">{t('confessions')}</h4>
                        ...
                        {locationCard.specials.map((mass) => (
                          <div key={mass.id} className="mb-3 p-4 rounded-lg bg-gradient-to-r from-[var(--primary)]/10 to-[var(--secondary)]/10 border-l-4 border-[var(--primary)]">
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                              {new Date(mass.date).toLocaleDateString(dateLocale, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} at {mass.time}
                            </p>
                            {mass.description && (
      ...
      {gallery.length > 0 && (
        <section id="gallery" className="py-20 bg-white text-center">
          <div className="max-w-6xl mx-auto px-4 sm:px-5">
            <span className="text-sm font-semibold text-[var(--primary)] uppercase tracking-widest">{t('gallery')}</span>
            <h2 className="text-3xl md:text-4xl my-6 text-[var(--secondary)]" style={{ fontFamily: 'var(--font-heading)' }}>
              {t('photoGallery')}
            </h2>
